...
Signals 和 State 之间的主要区别在于 Signals 返回一个 _getter_ 和一个 _setter_ ，而非响应式系统返回其值（和一个 _setter_ ）。_注意: 有些响应式系统一起返回 getter/setter，有些则作为两个独立的引用，但其思想是相同的。_
...
为什么返回 _getter_ 比返回其值要有优势？因为通过返回 _getter_ ，你可以将**状态引用**的传递与**状态值**的读取分离开来。
...
Signals 是在应用程序中存储状态的一种方式，类似于 React 中的 `useState()`。然而，最关键的区别在于，Signals 返回的是 _getter_ 和 _setter_ ，而非响应式系统只返回其 _value_ 和 _setter_ 。

这很这点恰恰很重要，因为 Signals 是响应式的，这意味着它们需要追踪对状态感兴趣的用户，并将状态更改通知订阅者。这是通过观察调用 _state-getter_ 的上下文来实现的，它创建了一个订阅。

相反，React 中的 `useState()` 只返回 _state-value_ ，这意味着它不知道组件如何使用它的 _state-value_ ，所以必须重新呈现整个组件树以响应状态的更改。

近年来，Signals 已经拥有了良好的 DX，这使得它们并不比传统系统难用。出于这个原因，我认为你将使用的下一个框架会是基于 Signals 的响应式框架。
